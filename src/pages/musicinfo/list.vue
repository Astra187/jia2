	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					<div class="lable">歌曲风格：</div>
					<el-input v-model="formSearch.songstyle" placeholder="歌曲风格" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item" >
					<div class="lable">歌手姓名：</div>
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('musicinfo','新增')" type="primary" @click="add('/index/musicinfoAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','歌曲名称')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						歌曲名称
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','歌曲风格')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						歌曲风格
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','播放量')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont " ></span>
						播放量
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','分享量')" type="success" @click="chartDialogShow4">
						<span class="icon iconfont " ></span>
						分享量
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','评论数')" type="success" @click="chartDialogShow5">
						<span class="icon iconfont " ></span>
						评论数
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('musicinfo','收藏量')" type="success" @click="chartDialogShow6">
						<span class="icon iconfont " ></span>
						收藏量
					</el-button>
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						...
						<div class="infoBox">
							...
							<div class="name">{{item.albumtitle}}</div>
		...
		<el-dialog
			...
			width="800">
			<div id="songstyleChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="playcountChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="sharecountChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="commentcountChart5" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="favcountChart6" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.songstyle != ''&&this.formSearch.songstyle != undefined) searchWhere.songstyle = '%' + this.formSearch.songstyle + '%';
				if (this.formSearch.singer != ''&&this.formSearch.singer != undefined) searchWhere.singer = '%' + this.formSearch.singer + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`musicinfo/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/musicinfoDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("wordCloud/musicinfo/songname",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var songstyleChart2 = echarts.init(document.getElementById("songstyleChart2"),'macarons');
					let params = {}
					this.$http.get("musicinfo/group/songstyle",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].songstyle);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].songstyle
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							songstyleChart2.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								songstyleChart2.resize();
							};
						}
					})
				})
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var playcountChart3 = echarts.init(document.getElementById("playcountChart3"),'macarons');
					let params = {}
					this.$http.get("musicinfo/value/songname/playcount",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							playcountChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								playcountChart3.resize();
							};
						}
					})
				})
			...
			chartDialog4() {
				this.$nextTick(()=>{
					var sharecountChart4 = echarts.init(document.getElementById("sharecountChart4"),'macarons');
					let params = {}
					this.$http.get("musicinfo/value/songname/sharecount",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							sharecountChart4.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								sharecountChart4.resize();
							};
						}
					})
				})
			...
			chartDialog5() {
				this.$nextTick(()=>{
					var commentcountChart5 = echarts.init(document.getElementById("commentcountChart5"),'macarons');
					let params = {}
					this.$http.get("musicinfo/value/songname/commentcount",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							commentcountChart5.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								commentcountChart5.resize();
							};
						}
					})
				})
			...
			chartDialog6() {
				this.$nextTick(()=>{
					var favcountChart6 = echarts.init(document.getElementById("favcountChart6"),'macarons');
					let params = {}
					this.$http.get("musicinfo/value/songname/favcount",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							favcountChart6.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								favcountChart6.resize();
							};
						}
					})
				})
			...
			discussClick (row){
				...
				this.$router.push({path: '/index/discussmusicinfo', query: params});
			},
			chapterClick (row){
				let params = {
					refid: row.id
				...
				this.$router.push({path: '/index/chaptermusicinfo', query: params});
			},
		}
	}
</script>
