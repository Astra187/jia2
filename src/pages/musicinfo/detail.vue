	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/musicinfo?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.songstyle}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.albumtitle}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">收藏量</div>
						<div class="text "  >{{detail.favcount}}</div>
					</div>
					<div class="item">
						<div class="lable">分享量</div>
						<div class="text "  >{{detail.sharecount}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">歌曲数</div>
						<div class="text "  >{{detail.songcount}}</div>
					</div>
					<div class="item">
						<div class="lable">歌单</div>
						<div class="text "  >{{detail.playlist}}</div>
					</div>
					<div class="item">
						<div class="lable">时长</div>
						<div class="text "  >{{detail.durtime}}</div>
					</div>
					<div class="item">
						<div class="lable">介绍</div>
						<div class="text "  >{{detail.albumdesc}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('musicinfo','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('musicinfo','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('musicinfo','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('musicinfo','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('musicinfo/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'musicinfo', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/musicinfo', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page,type=1) {
				this.$http.get('discussmusicinfo/list', {
					params: {
						page, 
						limit: this.pageSize, 
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussmusicinfo/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussmusicinfo/delete',[id]).then(res=>{
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussmusicinfo/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
						}
			...
			// 修改
			editClick(){
				this.$router.push(`/index/musicinfoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此音乐信息？') .then(_ => {
					this.$http.post('musicinfo/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
